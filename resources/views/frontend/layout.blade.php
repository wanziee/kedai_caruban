...
<body class="bg-gray-100">
    ...
    <nav class="md:hidden fixed bottom-1 width-full inset-x-4 text-white h-20 z-50 mb-2 flex gap-2 justify-around" x-data="{ cartCount: 0 }" x-init="cartCount = JSON.parse(localStorage.getItem('cart') || '[]').reduce((sum, item) => sum + item.qty, 0); window.addEventListener('cart-updated', () => { cartCount = JSON.parse(localStorage.getItem('cart') || '[]').reduce((sum, item) => sum + item.qty, 0); });">
        <div class="flex justify-around w-full h-full bg-primary items-center rounded-full shadow-lg p-2">
            ...
            <a href="{{ route('order') }}"
                class="flex flex-col items-center space-y-1 px-4 py-2 w-full h-full rounded-full  {{ request()->routeIs('order') ? 'bg-primary-light' : '' }} relative">
        ...
        <div
            class="flex justify-center bg-primary w-20 h-20 items-center rounded-full shadow-lg p-2 {{ request()->routeIs('search') ? 'bg-primary-light' : 'bg-primary' }}">
            <a href="{{ route('search') }}"
                ...
                <svg fill="#000000"  viewBox="0 0 24 24" id="search"
                    ...
                    <circle id="secondary" cx="10" cy="10" r="7"
                        style="fill: {{ request()->routeIs('search') ? 'gray' : 'var(--primary-color)' }}; stroke-width: 2;"></circle>
