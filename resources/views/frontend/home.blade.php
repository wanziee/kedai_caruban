                ...
                <div
                    ...
                    @foreach ($categories as $category)
                        <button type="button" @click="selectedCategory = '{{ strtolower($category->name) }}'"
                            ...
                            <span
                                :class="selectedCategory === '{{ strtolower($category->name) }}'
                                    ?
                                    'text-green-800 font-bold' :
                                    'text-gray-700 font-medium group-hover:text-green-600'"
                                class="text-xs md:text-base transition-colors text-center truncate w-full">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4">
                    ...
                    @foreach ($menuItems as $item)
                        <div x-show="selectedCategory === 'all' || selectedCategory === '{{ strtolower($item->category->name) }}'"
                            class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition flex flex-col @if(!$item->is_available) opacity-75 @endif">

                            @if ($item->image)
                                <img src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->name }}"
                                    class="w-full h-48 object-cover @if(!$item->is_available) grayscale @endif">
                            @else
